...
## 11b. Recent small world models (2024–2026) — focus for this project

| Model | Date | Org | Size | Approach | Result | Source |
|---|---|---|---|---|---|---|
| DRAMA | Oct 2024 | — | ~7M | Mamba (state-space) latent dynamics | matches larger MBRL; trainable on a laptop | arXiv:2410.08893 |
| Δ-IRIS | Jun 2024 | Micheli et al. | 25M | token world model, 4 tokens/frame | SOTA Crafter; ~10× faster than IRIS | arXiv:2406.19320 |
| Data-efficient Transformer WM | Feb 2025 | Google DeepMind | small | transformer, train in imagination (Dyna) | beats DreamerV3 and human expert on Craftax @1M steps | arXiv:2502.01591 |
| DINO-WM | Nov 2024 | NYU / Meta | frozen DINOv2 + small head | no decoder; predict future DINOv2 features | zero-shot goal planning, no rewards/demos | arXiv:2411.04983 |
| DINO-world ("Back to the Features") | Jul 2025 | Meta FAIR | n/r | no decoder; future frames in DINOv2 latent | SOTA video prediction, intuitive physics | arXiv:2507.19468 |
| V-JEPA 2-AC | Jun 2025 | Meta | 300M | no decoder; action-conditioned latent | zero-shot robot pick-and-place from <62 h video | arXiv:2506.09985 |
| V-JEPA 2 | Jun 2025 | Meta | 1.2B | no decoder; video latent prediction | SOTA motion understanding / action anticipation | arXiv:2506.09985 |

Two directions matter most for tiny world models: (1) **no decoder** — predict representations instead of pixels (JEPA and DINO-feature models, mostly Meta). Yann LeCun left Meta in late 2025 to start a world-model company (AMI Labs, unveiled Mar 2026) built on this idea — *secondary sources*. (2) **cheaper backbones** — state-space (Mamba) models such as DRAMA (7M, laptop-trainable). Both avoid the cost of generating pixels.

## 11c. Google: Gemini vs Genie (stated accurately)

- **Gemini is not itself a world model.** At Google I/O (May 2025) Google said it is **extending Gemini toward a world model** that can plan and simulate. Roadmap/vision statement, not a shipped world-model product.
- **Genie 2 (Dec 2024)** and **Genie 3 (Aug 2025)** are DeepMind's actual generative interactive world models. Genie 3: real time, 720p, 24 FPS, multi-minute consistency. **No model size or compute disclosed.**
- **SIMA 2 (Nov–Dec 2025)** is a Gemini-based embodied agent (built on Gemini 2.5) that trains and self-improves inside worlds generated by Genie 3 — the accurate way to describe the Gemini↔world-model link. (arXiv:2512.04797)
- Do **not** put a parameter count or compute figure on Gemini, Genie, or SIMA — none are disclosed.
